...
class ComplexValueObject extends ValueObject {
    constructor(readonly prop1: string, readonly prop2: number) { super(); }
}

describe('ValueObject Unit Tests', () => {
    test('should be equals', () => {
        const testValue = 'test';
        const valueObject1 = new StringValueObject(testValue);
        const valueObject2 = new StringValueObject(testValue);
        expect(valueObject1.equals(valueObject2)).toBeTruthy();

        const complexValueObject1 = new ComplexValueObject(testValue, 1);
        const complexValueObject2 = new ComplexValueObject(testValue, 1);
        expect(complexValueObject1.equals(complexValueObject2)).toBeTruthy();
         
    });
    test('should not be equals', () => {
        const valueObject1 = new StringValueObject('test');
        const valueObject2 = new StringValueObject('tes2');
        ...
        const testValue = 'test';
        const complexValueObject1 = new ComplexValueObject(testValue, 1);
        const complexValueObject2 = new ComplexValueObject(testValue, 2);
